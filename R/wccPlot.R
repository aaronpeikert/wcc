...
wccPlot <- function(inSeries1=NA, inSeries2=NA, startwindow=1, endwindow=200, wMax=50, tMax=50, wInc=1, tInc=1, Lsize=8, pspan=.25, type="Max", samplespersecond=1, windcross=TRUE, vectorfield=FALSE) {


    if (!is.numeric(inSeries1) | !is.numeric(inSeries2) ) {
        stop(paste0("Warning: inSeries1 and inSeries2 must be of type numeric."))
    }
    if ( !is.vector(inSeries1) | !is.vector(inSeries2) | length(inSeries1) != length(inSeries2)) {
    ...
    if (vectorfield==TRUE) {
        VFout <- wccVectorFieldCalc(tAllCor=wccMatrix, type=type)
    }
    else {
